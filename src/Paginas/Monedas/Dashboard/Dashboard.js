...
export const Dashboard = () => {
  ...
  const urlMoneda = `https://api.coingecko.com/api/v3/coins/${id}/market_chart?vs_currency=usd&days=7`;
  const { data } = useFetch(urlMoneda);


  const coinChartData = dataParaChart&&dataParaChart.prices.map((value) => ({
    x: value[0],
    y: value[1].toFixed(2),
  }));
        ...
        <div className="dashboard-cuadro-info">
          <div className="dashboard-cuadro-info-nombres">
            <p className="dashboard-cuadro-info-nombres-nombre">
              {dataMonedaElegida.name}
            </p>
            <p className="dashboard-cuadro-info-nombres-symbol">
              {dataMonedaElegida.symbol}
            </p>
          </div>
          <div>
            <p className="dashboard-cuadro-info-nombres-precio">
              US$ {dataMonedaElegida.market_data.current_price.usd.toFixed(2)}
            </p>
